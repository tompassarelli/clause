...
/// A qualified Clause name.  Roles and entity locals additionally require a
/// single segment, enforced by their owning constructors.
#[derive(Clone, Debug, Eq, Ord, PartialEq, PartialOrd)]
pub struct Name(String);

impl Name {
    pub fn new(value: String) -> Result<Self> {
        if value.split('/').all(valid_segment) && !value.is_empty() {
            Ok(Self(value))
        } else {
    ...
    pub fn new(model: ModelId, local: Name, typ: TypeId) -> Result<Self> {
        if !local.is_local() {
            return Err(KernelError::new(
                "entity local name must be a single segment",
            ));
        }
