...
mod tests {
    ...
    fn sha256(input: &[u8]) -> [u8; 32] {
        ...
        for block in padded.chunks_exact(64) {
            ...
            for (index, word) in block.chunks_exact(4).take(16).enumerate() {
                words[index] =
                    u32::from_be_bytes(word.try_into().expect("SHA-256 word has four bytes"));
            }
            for index in 16..64 {
            ...
            for (constant, word) in ROUND_CONSTANTS.into_iter().zip(words) {
                let choice = (e & f) ^ (!e & g);
    ...
    fn slash_qualified_fixture_remains_classified_and_quarantined() {
        ...
        assert_eq!(sha256_hex(SOURCE.as_bytes()), HISTORICAL_FIXTURE_SHA256);
