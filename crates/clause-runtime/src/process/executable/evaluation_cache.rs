...
fn dependencies(expression: &ExecutableExpressionV1, reads: &mut BTreeSet<u16>) -> bool {
    ...
    match expression {
        ...
        E::SequenceSort(value) | E::TextCodepoint(value) | E::TextFromCodepoint(value) | E::TextCharacters(value) | E::ParseIntegerPrefix(value) | E::SequenceRange(value) | E::SequenceCount(value) | E::ScalarText(value) | E::Field(value,_) => dependencies(value,reads),
        ...
        E::Foreign { .. } => false,
        E::Match { value, cases } => {
            let mut cacheable = dependencies(value, reads);
            for (_, _, body) in cases { cacheable &= dependencies(body, reads); }
            cacheable
        }
