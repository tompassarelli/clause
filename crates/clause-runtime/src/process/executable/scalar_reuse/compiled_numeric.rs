...
fn constrain(
    ...
    match nodes[index].0 {
        ...
        Node::Equal(a, b) => {
            let operand = known(a, nodes)?;
            if known(b, nodes)? != operand {
                return None;
            }
            child(a, operand)?;
            child(b, operand)?;
    ...
    fn fresh(&mut self, node: usize) {
        ...
        match self.plan.nodes[node].0 {
            ...
            Node::Equal(a, b) => {
                ...
                self.op(if self.kinds[a] == Kind::Number {
                    W::I64Eq
                } else {
                    W::I32Eq
                });
...
impl NumericQuery {
    pub(super) fn compile(plan: &ScalarPlan) -> Option<Arc<Self>> {
        ...
        code.function(&finite).function(&emitter.function);
        module.section(&code);
        Some(Arc::new(Self {
...
impl NumericQuery {
    fn pack(
        ...
        let write = |data: &mut [u8], input: &Input, value: Result<&ExecutableValueV1, u32>| {
