...
pub struct CanonicalStateRefV1 {
    ...
    pub subject: Vec<u8>,
    /// Exact typed identity of this state subject's declared domain facet.
...
pub enum CanonicalExecutableExpressionV1 {
    ...
    TextCodepoint(Box<Self>),
    TextFromCodepoint(Box<Self>),
    /// Literal splitting preserves empty fields; an empty delimiter selects characters.
    TextSplit(Box<Self>, Box<Self>),
    /// Decimal prefix yields finite F64 or the original rejected Text.
    ...
    Accumulate(Box<Self>),
    MatchesAny(Vec<Vec<CanonicalExecutablePredicateV1>>),
    Not(Box<Self>),
    FreshReferent {
        domain: FormationLocalId,
        binder: u16,
...
pub enum CanonicalScalarExpressionV1 {
    ...
    SequenceFold { accumulator: Vec<u8>, item: Vec<u8>, source: Box<Self>, initial: Box<Self>, body: Box<Self> },
    SequenceAppend(Box<Self>, Box<Self>),
    SequenceSort(Box<Self>),
    Record(BTreeMap<Vec<u8>, Self>),
    Field(Box<Self>, Vec<u8>),
...
struct GeneralHandlerCst {
    ...
    binding_constraints: Vec<BindingConstraintCst>,
    subject: Vec<u8>,
    ...
    accumulations: Vec<GeneralAssignmentCst>,
...
struct GeneralReplacementCst {
    ...
    aggregate_result: Option<GeneralAggregateResultCst>,
    required_sources: Vec<ScalarParameterSourceCst>,
}

struct GeneralAggregateResultCst {
    parameter: Vec<u8>,
    shape: Vec<u8>,
...
struct ScalarHandlerCase {
    subject: Vec<u8>,
    components: Vec<ScalarHandlerComponentCase>,
}

#[derive(Clone)]
struct ScalarHandlerComponentCase {
    field: Option<Vec<u8>>,
...
fn scalar_parts_use_initial(
    parts: &ScalarHandlerParts<'_>,
    origin: CanonicalSourceOriginV1,
) -> bool {
    parts
        .cases
        ...
        .any(|component| component.initial_origin == origin)
}

fn scalar_parts_use_parameter(
    parts: &ScalarHandlerParts<'_>,
    subject: &[u8],
    relation: &[u8],
    has_field: bool,
    ...
    parts
        ...
        .iter()
        .flat_map(|case| &case.components)
        .flat_map(|component| &component.parameter_sources)
        .any(|source| {
            source.subject == subject
                && source.relation == relation
                && source.field.is_some() == has_field
        })
}
...
pub fn read_canonical_source_with_imports_and_frontend_v1(
    ...
    while cursor < lines.len() {
        ...
        if let Some(edges) = patterns::edge_focus(&logical_source_lines(artifact, block)?.into_iter()
            .filter(|line| !line.text.is_empty()).collect::<Vec<_>>(), frontend)? {
            for edge in edges {
    ...
    for item in &cst.items {
        ...
        match &item.kind {
            ...
            CstKind::GeneralHandler(handler) => {
                ...
                if !handler.insertions.is_empty() || !handler.membership_sources.is_empty() {
                    for concrete_subject in selected_general_handler_subjects(cst, handler)? {
                        for insertion in &handler.insertions {
                            let request = AllocationRequest {
                                producer: assertion_producer(
                                    &concrete_subject,
...
fn general_handler_relation_designations(
    ...
    handler
        .parameter_sources
        .iter()
        .chain(handler.selectors.iter().map(|selector| &selector.source))
...
fn selected_general_handler_subjects(
    cst: &CanonicalSourceCstV1,
    handler: &GeneralHandlerCst,
) -> Result<Vec<Vec<u8>>, CanonicalSourceErrorV1> {
    let subjects = concrete_general_handler_subjects(cst, handler)?;
    let has_indirect_variable_source = handler
        .parameter_sources
        .iter()
        .chain(handler.selectors.iter().map(|selector| &selector.source))
        .chain(&handler.membership_sources)
        ...
        .any(|source| {
            source.subject.starts_with(b"?")
                && source.subject != handler.subject
                && !handler
                    .arguments
                    .iter()
                    .any(|argument| argument.designation == source.subject)
        });
    if subjects.len() > 1
        && handler.selectors.is_empty()
        && handler.predicates.is_empty()
        && handler.boolean_conditions.is_empty()
        && has_indirect_variable_source
    {
...
fn declared_referent_value(
    ...
    if !declared_domain_facet(cst, designation, domain) {
        return Err(CanonicalSourceErrorV1::MissingExecutableBinding { origin });
    }
    let producer = semantic_producer(CanonicalSourceProductionV1::Referent, designation);
...
fn input_subject_identity(
    ...
    declared_domain_facet(cst, subject, relation.subject_domain)
...
fn typed_state_value(
    ...
    let CanonicalScalarValueV1::Symbol(designation) = &value else {
        return Ok(value);
    };
    let relation = cst
        .items
        .iter()
        .find_map(|item| match &item.kind {
            CstKind::Relation(relation) if relation.designation == state.relation_designation => {
                Some(relation)
            }
            _ => None,
        })
    ...
    if state.path == CanonicalStatePathV1::Scalar
        && declared_domain_facet(cst, designation, relation.value_domain)
    {
        return Ok(CanonicalScalarValueV1::Referent(declared_referent_value(
            cst,
            plan,
            designation,
            relation.value_domain,
...
fn numeric_contribution_expression(
    cst: &CanonicalSourceCstV1,
    plan: &CanonicalSourceAllocationPlanV1,
    expression: &CanonicalExecutableExpressionV1,
    referent_arguments: &BTreeSet<u16>,
    origin: CanonicalSourceOriginV1,
) -> Result<bool, CanonicalSourceErrorV1> {
    use CanonicalExecutableExpressionV1 as E;
    Ok(match expression {
        E::Constant(CanonicalScalarValueV1::Number(_)) => true,
        E::Argument(ordinal) => !referent_arguments.contains(ordinal),
        E::State(state) => {
            let relation = cst
                .items
                .iter()
                .find_map(|item| match &item.kind {
                    CstKind::Relation(relation)
                        if relation.designation == state.relation_designation =>
                    {
                        Some(relation)
                    }
                    _ => None,
            ...
            let relation = resolved_state_relation_for(plan, relation, origin)?;
            match &state.path {
                CanonicalStatePathV1::Scalar => relation.value_domain == b"F64",
                CanonicalStatePathV1::Field { designation, .. } => {
                    cst.items.iter().any(|item| matches!(&item.kind,
                        CstKind::Shape { designation: shape, fields }
                            if shape == relation.value_domain && fields.iter().any(|field| field.name == *designation && field.domain == b"F64")))
                }
                _ => false,
            }
        }
        E::Add(a, b) | E::Subtract(a, b) | E::Multiply(a, b) | E::Divide(a, b) => {
            numeric_contribution_expression(cst, plan, a, referent_arguments, origin)?
                && numeric_contribution_expression(cst, plan, b, referent_arguments, origin)?
        }
        _ => false,
    })
}
...
fn canonical_relation_table(
    ...
    for item in &cst.items {
        ...
        if cardinality != CanonicalRelationCardinalityV1::Many
...
fn resolve_general_parameter_states(
    ...
    initial_entities: BTreeMap<Vec<u8>, Vec<u8>>,
    origin: CanonicalSourceOriginV1,
    allow_independent_subjects: bool,
    independent_arguments: &BTreeMap<Vec<u8>, Vec<u8>>,
...
struct ResolvedBooleanDerive<'a> {
    ...
    bindings: BTreeMap<Vec<u8>, Vec<u8>>,
    value: bool,
    cases: Vec<ResolvedBooleanDeriveCase>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
struct ResolvedBooleanDeriveCase {
    parameters: BTreeMap<Vec<u8>, CanonicalStateRefV1>,
    selector_equalities: Vec<(CanonicalStateRefV1, CanonicalScalarValueV1)>,
}
...
fn resolved_boolean_derives<'a>(
    ...
    let derives = cst
        ...
        .collect::<Vec<_>>();
    let mut resolved: Vec<ResolvedBooleanDerive<'a>> = Vec::with_capacity(derives.len());
    for derive in derives {
        ...
        let mut sources = law.parameter_sources.clone();
        let mut selector_parameters = Vec::with_capacity(law.selectors.len());
        for (index, selector) in law.selectors.iter().enumerate() {
            validate_scalar_state_selector(cst, plan, selector)?;
            let parameter = format!("\0boolean-selector-{index}").into_bytes();
            let mut source = selector.source.clone();
            source.parameter = parameter.clone();
            sources.push(source);
            selector_parameters.push((parameter, selector.expected.clone()));
        }
        ...
        for solution in solutions {
            let bindings = concretize_relation_bindings(
                &result.bindings,
                &solution.entities,
                law.result.origin,
            )?;
            let subject = bindings.get(subject_role).ok_or(
                CanonicalSourceErrorV1::MissingExecutableBinding {
                    origin: law.result.origin,
                },
            )?;
            ...
            let mut selector_equalities = solution
                .selector_equalities
                .into_iter()
                .map(|(selector, expected)| (selector, CanonicalScalarValueV1::Symbol(expected)))
                .collect::<Vec<_>>();
            for (parameter, expected) in &selector_parameters {
                let selector = solution.parameters.get(parameter).cloned().ok_or(
                    CanonicalSourceErrorV1::MissingExecutableBinding { origin: law.origin },
                )?;
                selector_equalities.push((selector, expected.clone()));
            }
            selector_equalities.sort();
            selector_equalities.dedup();
            let case = ResolvedBooleanDeriveCase {
                parameters: solution.parameters,
                selector_equalities,
            };
            if let Some(existing) = resolved.iter_mut().find(|candidate| {
                candidate.derive.designation == derive.designation
                    && candidate.state == state
                    && candidate.bindings == bindings
                    && candidate.value == result.value
            }) {
                if !existing.cases.contains(&case) {
...
fn validate_scalar_state_selector(
    cst: &CanonicalSourceCstV1,
    plan: &CanonicalSourceAllocationPlanV1,
...
fn validate_scalar_selector_value(
    ...
    let valid = match (&selector.expected, domain.as_slice()) {
        ...
        (CanonicalScalarValueV1::Symbol(value), domain)
            if !matches!(domain, b"F64" | b"Bool" | b"Text") =>
        {
            declared_referent_value(cst, plan, value, domain, selector.origin)?;
            true
        }
        _ => false,
    };
...
fn checked_source_state_cells(
    ...
    for item in &cst.items {
        ...
        match &item.kind {
            ...
            CstKind::SymbolAssertion(assertion) => {
                ...
                let value_kind = if matches!(value, CanonicalScalarValueV1::Referent(_)) {
                    CanonicalScalarValueKindV1::Referent
                } else {
                    CanonicalScalarValueKindV1::Symbol
                };
                cells.insert(
                    state.clone(),
                    CanonicalStateCellV1 {
    ...
    for handler in cst.items.iter().filter_map(|item| match &item.kind {
        ...
        for subject in selected_general_handler_subjects(cst, handler)? {
            for insertion in &handler.insertions {
                ...
                match state_relation_cardinality(cst, plan, &insertion.target, handler.origin)? {
                    SourceCardinality::Maybe => {
                        require_optional_state_relation(
                            cst,
                            plan,
                            &insertion.target,
                            handler.origin,
...
fn canonical_scalar_executable_expression(
    ...
    Ok(match expression {
        ...
        | CanonicalScalarExpressionV1::TextCodepoint(_) | CanonicalScalarExpressionV1::TextFromCodepoint(_) | CanonicalScalarExpressionV1::TextCharacters(_)
        | CanonicalScalarExpressionV1::TextSplit(_, _)
...
fn checked_executable_handlers(
    ...
    for derive in &derives {
        ...
        for case in &derive.cases {
            ...
            predicates.extend(case.selector_equalities.iter().map(|(selector, expected)| {
                CanonicalExecutablePredicateV1::Equal(
                    CanonicalExecutableExpressionV1::State(selector.clone()),
                    constant_expression(expected.clone()),
                )
            }));
            for predicate in &mut predicates {
                normalize_typed_predicate(cst, plan, predicate, derive.law.origin)?;
            }
            cases.push(predicates);
        }
        let value = CanonicalExecutableExpressionV1::MatchesAny(cases);
        rules.push(CanonicalExecutableRuleV1 {
            ...
            assignments: vec![CanonicalExecutableAssignmentV1 {
                ...
                value: if derive.value {
                    value
                } else {
                    CanonicalExecutableExpressionV1::Not(Box::new(value))
                },
            }],
            removals: vec![],
        });
    ...
    for parts in scalar {
        ...
        for case in &parts.cases {
            ...
            let mut entities =
                BTreeMap::from([(parts.handler.subject.clone(), case.subject.clone())]);
            for component in &case.components {
                let current = state_ref_for_origin(
                    cst,
                    plan,
                    component.initial_origin,
                    component.field.as_deref(),
                )?;
                let (parameters, component_entities) = resolve_parameter_states(
                    cst,
                    plan,
                    &component.parameter_sources,
                    entities.clone(),
                    parts.handler.origin,
                )?;
                for (parameter, subject) in component_entities {
                    if let Some(previous) = entities.insert(parameter, subject.clone())
                        && previous != subject
                    {
                        return Err(CanonicalSourceErrorV1::AmbiguousExecutableBinding {
                ...
                if assignments.is_empty() {
                    predicates.extend(canonical_scalar_executable_predicates(
            ...
            assignments.sort_by(|left, right| left.target.cmp(&right.target));
            let key = assignments
                .first()
                .map(|assignment| assignment.target.clone())
                .ok_or(CanonicalSourceErrorV1::InvalidScalarHandler {
                    origin: parts.handler.origin,
                })?;
    ...
    for source in cst.items.iter().filter_map(|item| match &item.kind {
        ...
        selected_general_handler_subjects(cst, source)?;
        let mut parameter_sources = source.parameter_sources.clone();
        let mut selector_parameters = Vec::with_capacity(source.selectors.len());
        for (index, selector) in source.selectors.iter().enumerate() {
            validate_scalar_state_selector(cst, plan, selector)?;
            let parameter = format!("\0general-selector-{index}").into_bytes();
            let mut selector_source = selector.source.clone();
            selector_source.parameter = parameter.clone();
            parameter_sources.push(selector_source);
            selector_parameters.push((parameter, selector.expected.clone()));
        }
        let current_source = source
            .assignments
            .first()
            .or_else(|| source.accumulations.first())
            .map(|assignment| &assignment.target)
            ...
            .clone();
        let current_parameter = b"\0general-current".to_vec();
        let mut current_parameter_source = current_source;
        current_parameter_source.parameter = current_parameter.clone();
        parameter_sources.push(current_parameter_source);
        for (index, accumulation) in source.accumulations.iter().enumerate() {
            let relation =
                resolved_state_relation(cst, plan, &accumulation.target.relation, source.origin)?;
            if general_source_value_domain(cst, &relation, &accumulation.target, source.origin)?
                != b"F64"
                || !singleton_forward_mode(&relation)
            {
                return Err(CanonicalSourceErrorV1::InvalidGeneralHandler {
                    origin: source.origin,
                });
            }
            let mut target = accumulation.target.clone();
            target.parameter = format!("\0general-accumulate-{index}").into_bytes();
            parameter_sources.push(target);
        }
        let independent_arguments = cst
            .items
            .iter()
            .filter_map(|item| match &item.kind {
                CstKind::ReferentInputBinding(binding)
                    if binding.handler_designation == source.designation
                        && source.arguments.len() == 1 =>
                {
                    Some((
                        source.arguments[0].designation.clone(),
        ...
        for scalar_case in binding_cases(&source.scalar_bindings)? {
            ...
            for solution in solutions.iter().cloned() {
                ...
                for (variable, subject) in &entities {
                    if let Some(identity) = parameter_sources
                        .iter()
                        .filter(|source| source.subject == *variable)
                        .filter_map(|source| parameters.get(&source.parameter))
                        .find_map(|state| {
                ...
                for (argument, domain) in &independent_arguments {
                    if let Some(subject) = entities.get(argument) {
                        let expected =
                            declared_referent_value(cst, plan, subject, domain, source.origin)?;
                        predicates.push(CanonicalExecutablePredicateV1::Equal(
                            CanonicalExecutableExpressionV1::Argument(arguments[argument]),
                            constant_expression(CanonicalScalarValueV1::Referent(expected)),
                        ));
                    }
                }
                for (selector, expected) in selector_equalities {
                    let expected = typed_state_value(
                        cst,
                        plan,
                        &selector,
                        CanonicalScalarValueV1::Symbol(expected),
                        source.origin,
                    )?;
                    predicates.push(CanonicalExecutablePredicateV1::Equal(
                ...
                for (parameter, expected) in &selector_parameters {
                    ...
                    let expected =
                        typed_state_value(cst, plan, &selector, expected.clone(), source.origin)?;
                    predicates.push(CanonicalExecutablePredicateV1::Equal(
                        CanonicalExecutableExpressionV1::State(selector),
                        constant_expression(expected),
                    ));
                ...
                required_present.sort();
                required_present.dedup();
                for accumulation in &source.accumulations {
                    let target = general_target_state_ref(
                        cst,
                        plan,
                        &accumulation.target,
                        &entities,
                    ...
                    if assignments
                        .iter()
                        .any(|assignment| assignment.target == target)
                    ...
                    let referent_arguments = independent_arguments
                        .keys()
                        .map(|name| arguments[name])
                        .collect();
                    if !numeric_contribution_expression(
                        cst,
                        plan,
                        &delta,
                        &referent_arguments,
                    ...
                    assignments.push(CanonicalExecutableAssignmentV1 {
                        target,
                        value: CanonicalExecutableExpressionV1::Accumulate(Box::new(delta)),
                    });
                }
                rules.push(CanonicalExecutableRuleV1 {
    ...
    for handler in &mut handlers {
        for rule in &mut handler.rules {
            for assignment in &mut rule.assignments {
                if let CanonicalExecutableExpressionV1::Constant(value) = &mut assignment.value {
                    *value = typed_state_value(
                        cst,
                        plan,
                        &assignment.target,
                        value.clone(),
            ...
            for predicate in &mut rule.predicates {
                normalize_typed_predicate(cst, plan, predicate, cst.items[0].origin)?;
            }
        }
    }
    Ok(handlers)
...
fn normalize_typed_predicate(
    cst: &CanonicalSourceCstV1,
    plan: &CanonicalSourceAllocationPlanV1,
    predicate: &mut CanonicalExecutablePredicateV1,
    origin: CanonicalSourceOriginV1,
) -> Result<(), CanonicalSourceErrorV1> {
...
fn checked_canonical_source_execution_v1(
    ...
    let scalar_handlers = scalar_parts
        .iter()
        .flat_map(|parts| {
            parts.cases.iter().filter_map(|case| {
                let [component] = case.components.as_slice() else {
                    return None;
                };
                Some(CanonicalScalarHandlerV1 {
                    artifact: cst.artifact,
                    handler_origin: parts.handler.origin,
                    initial_assertion_origin: component.initial_origin,
                    include_origin: parts.handler.include.origin,
                    initial_value: component.initial_value.clone(),
...
fn parse_general_handler(
        ...
        .filter(|line| !line.text.trim().is_empty())
    {
        ...
        match section.as_str() {
            ...
            "accumulate" => accumulate.push(entry),
            _ => return Ok(None),
        }
    }
    if let Some((parameter, _)) = pending_creation.take() {
    ...
    for clause in &when {
        ...
        for mut source in sources {
            ...
            if parameter_sources.contains_key(&source.parameter) {
                let original = source.parameter.clone();
                let alias = format!(
                    "\0general-join-{}-{}",
                ...
                predicates.push(CanonicalScalarPredicateV1::Equal(
                    CanonicalScalarExpressionV1::Parameter(original),
                    CanonicalScalarExpressionV1::Parameter(alias.clone()),
                ));
                source.parameter = alias;
            }
            parameter_sources.insert(source.parameter.clone(), source);
        }
    }
    ...
    for (withdraw_index, withdraw) in withdraw.iter().enumerate() {
        ...
        if let Some(result) = &replacement.aggregate_result {
            let Some(source) = parameter_sources.remove(&result.parameter) else {
                return Ok(None);
            };
            if source.shape.is_some() || source.field.is_some() {
                return Ok(None);
            }
            for (field, parameter) in &result.fields {
                let mut component = source.clone();
                component.parameter = parameter.clone();
                component.shape = Some(result.shape.clone());
                component.field = Some(field.clone());
                if parameter_sources
                    .insert(parameter.clone(), component)
                    .is_some()
                {
                    return Ok(None);
                }
    ...
    for assignment in assignments.iter().chain(&accumulations) {
...
fn parse_general_assignments(
    ...
    if let Some((withdraw_prefix, withdraw_shape, withdraw_fields)) = split_shape_subject(withdraw)
    {
        if let Some(result_parameter) = include.strip_prefix(&format!("{withdraw_prefix} "))
            && result_parameter.starts_with('?')
            && result_parameter.split_whitespace().count() == 1
        {
            ...
            let fields = current
                ...
                .map(|(field, _)| {
                    (
                        ...
                        format!("\0general-aggregate-result-{field}").into_bytes(),
                    )
                })
                .collect::<Vec<_>>();
            ...
            return Some(GeneralReplacementCst {
                ...
                aggregate_result: Some(GeneralAggregateResultCst {
                    parameter: result_parameter.as_bytes().to_vec(),
                    shape: withdraw_shape.as_bytes().to_vec(),
                    fields,
                }),
...
impl ScalarExpressionParser<'_> {
    ...
    fn primary_value(&mut self) -> Option<CanonicalScalarExpressionV1> {
        ...
        for builtin in [b"characters(".as_slice(), b"codepoint(", b"from-codepoint(", b"parse-integer-prefix("] {
            if self.take_exact(builtin) {
                let value = Box::new(self.disjunction()?);
                self.skip_spaces(); self.take_exact(b")").then_some(())?;
                return Some(match builtin { b"characters(" => E::TextCharacters(value), b"codepoint(" => E::TextCodepoint(value), b"from-codepoint(" => E::TextFromCodepoint(value), _ => E::ParseIntegerPrefix(value) });
            }
        }
        if self.take_exact(b"split-text(") {
            let value = Box::new(self.disjunction()?);
            self.skip_spaces(); self.take_exact(b",").then_some(())?;
            let delimiter = Box::new(self.disjunction()?);
            self.skip_spaces(); self.take_exact(b")").then_some(())?;
...
fn collect_scalar_expression_parameters(
    ...
    match expression {
        ...
        CanonicalScalarExpressionV1::SequenceFold { accumulator, item, source, initial, body } => {
            collect_scalar_expression_parameters(source, parameters);
            collect_scalar_expression_parameters(initial, parameters);
            let mut nested = BTreeSet::new();
            collect_scalar_expression_parameters(body, &mut nested);
            nested.remove(accumulator); nested.remove(item);
        ...
        CanonicalScalarExpressionV1::TextCodepoint(value) | CanonicalScalarExpressionV1::TextFromCodepoint(value) | CanonicalScalarExpressionV1::TextCharacters(value) | CanonicalScalarExpressionV1::ParseIntegerPrefix(value) | CanonicalScalarExpressionV1::SequenceRange(value) | CanonicalScalarExpressionV1::SequenceCount(value) | CanonicalScalarExpressionV1::ScalarText(value) => collect_scalar_expression_parameters(value, parameters),
        ...
        | CanonicalScalarExpressionV1::StartsWith(left, right)
        | CanonicalScalarExpressionV1::ContainsText(left, right)
        | CanonicalScalarExpressionV1::TextSplit(left, right)
        | CanonicalScalarExpressionV1::GreaterThan(left, right)
        | CanonicalScalarExpressionV1::LessThanOrEqual(left, right)
        | CanonicalScalarExpressionV1::Concatenate(left, right)
        | CanonicalScalarExpressionV1::Add(left, right)
...
fn parse_boolean_law(
    ...
    let mut selectors = Vec::new();
    let mut predicates = Vec::new();
    for (condition, condition_origin) in conditions {
        if let Some(predicate) = parse_scalar_predicate(condition, "") {
            predicates.push(predicate);
            continue;
        }
        if let Some(sources) = parse_law_state_declaration(condition) {
            for source in sources {
                if parameter_sources
        ...
        let Some(selector) = parse_scalar_state_selector(condition, condition_origin) else {
            return Ok(None);
        };
        selectors.push(selector);
    }
...
fn parse_scalar_state_selector(
    source: &str,
    origin: CanonicalSourceOriginV1,
    ...
    let parts = source.split_whitespace().collect::<Vec<_>>();
    let expected = *parts.last()?;
    if parts.len() < 3 || expected.starts_with('?') {
        return None;
    }
    let expected = if let Some(number) = parse_source_number(expected) {
        ...
        CanonicalScalarValueV1::Symbol(designation_bytes(expected, origin).ok()?)
    };
...
fn parse_declaration(
    ...
    while cursor < lines.len() {
        ...
        let source = &line.text[2..];
        let mut end = cursor + 1;
        while end < lines.len() && lines[end].indent > 2 { end += 1; }
        if !source.contains('?') && end == cursor + 1 {
            let edge = frontend.edge(&designation, source, origin).map_err(|error| match error {
                CanonicalSourceErrorV1::MissingDeclaredProduction { .. } =>
                    CanonicalSourceErrorV1::InvalidDenotation { origin },
                error => error,
            })?;
        ...
        for clause in patterns::clauses(&logical, 2, frontend, &ScalarLawEnvironment::default())? {
            if let Some(constraint) = parse_binding_constraint(&clause.text, clause.origin)? {
                ...
                if let Some(previous) = roles.iter().find(|role: &&RelationRoleCst| role.name == name) {
                    if previous.domain != constraint.domain {
                        return Err(CanonicalSourceErrorV1::InvalidRelationChild { origin: clause.origin });
                    }
                } else {
                    roles.push(RelationRoleCst { name, domain: constraint.domain, origin: constraint.origin });
                }
            } else {
                if reading.is_some() {
                    return Err(CanonicalSourceErrorV1::InvalidRelationChild { origin: clause.origin });
    ...
    ensure_unique_children(&designation, fields.iter().map(|field| field.name.as_slice()))?;
    let origin = block_origin(artifact, block);
    let kind = match reading {
        None if !fields.is_empty() && roles.is_empty() => CstKind::Shape {
            designation,
            fields,
        },
        Some(reading) if fields.is_empty() => {
            let declared = roles
                .iter()
...
fn parse_binding_constraint(
    source: &str,
    origin: CanonicalSourceOriginV1,
) -> Result<Option<BindingConstraintCst>, CanonicalSourceErrorV1> {
    let Some((parameter, domain)) = source.split_once(" shape ") else { return Ok(None) };
    let Some(name) = parameter.strip_prefix('?') else { return Ok(None) };
    let name = denotation_designation_bytes(name, origin)?;
    Ok(Some(BindingConstraintCst {
        parameter: [b"?".as_slice(), &name].concat(),
        domain: application_designation_bytes(domain, origin)?,
        origin,
    }))
}

fn check_binding_constraints(
    constraints: &[BindingConstraintCst],
    domains: &BTreeMap<Vec<u8>, Vec<u8>>,
) -> Result<(), CanonicalSourceErrorV1> {
    for constraint in constraints {
        if domains.get(&constraint.parameter) != Some(&constraint.domain) {
            return Err(CanonicalSourceErrorV1::MissingExecutableBinding { origin: constraint.origin });
        }
    }
    Ok(())
}
...
fn scalar_handler_parts(
    ...
    for handler in handlers {
        if handler.field.is_none()
            && let Some((shape, fields)) = state_relation_shape(cst, &handler.relation)
        {
            let CanonicalScalarExpressionV1::Parameter(result_parameter) = &handler.result else {
                return Err(CanonicalSourceErrorV1::InvalidScalarHandler {
                    origin: handler.origin,
                });
            };
            let result_sources = handler
                .parameter_sources
                .iter()
                .filter(|source| source.parameter == *result_parameter)
                .collect::<Vec<_>>();
            let [result_source] = result_sources.as_slice() else {
                return Err(CanonicalSourceErrorV1::InvalidScalarHandler {
                    origin: handler.origin,
                });
            };
            let Some((result_shape, result_fields)) =
                state_relation_shape(cst, &result_source.relation)
            else {
                return Err(CanonicalSourceErrorV1::InvalidScalarHandler {
                    origin: handler.origin,
                });
            };
            if result_source.subject != handler.subject
                || result_source.shape.is_some()
                || result_source.field.is_some()
                || result_shape != shape
                || result_fields
                    .iter()
                    .map(|field| (&field.name, &field.domain))
                    .ne(fields.iter().map(|field| (&field.name, &field.domain)))
            {
                return Err(CanonicalSourceErrorV1::InvalidScalarHandler {
                    origin: handler.origin,
                });
            ...
            let mut cases = Vec::new();
            for item in &cst.items {
                let (subject, initial_origin, initial_values) = match &item.kind {
                    CstKind::VectorAssertion(assertion)
                        if assertion.relation == handler.relation =>
                    {
                        let values = fields
                            .iter()
                            .map(|field| match field.name.as_slice() {
                                b"x" => Some(CanonicalScalarValueV1::Number(assertion.x)),
                                b"y" => Some(CanonicalScalarValueV1::Number(assertion.y)),
                                b"z" => Some(CanonicalScalarValueV1::Number(assertion.z)),
                                _ => None,
                            })
                            .collect::<Option<Vec<_>>>()
                            .ok_or(CanonicalSourceErrorV1::InvalidScalarHandler {
                        ...
                        (assertion.subject.clone(), assertion.origin, values)
                    }
                    CstKind::ShapeAssertion(assertion)
                        if assertion.relation == handler.relation =>
                    {
                        if assertion.shape != shape {
                            return Err(CanonicalSourceErrorV1::InvalidScalarHandler {
                                origin: handler.origin,
                            });
                        }
                ...
                let components = fields
                    .iter()
                    .zip(initial_values)
                    .map(|(field, initial_value)| {
                        let parameter_sources = handler
                            .parameter_sources
                            .iter()
                            .cloned()
                            .map(|mut source| {
                                if source.parameter == *result_parameter {
                                    source.shape = Some(shape.to_vec());
                                    source.field = Some(field.name.clone());
                                }
                                source
                            })
                            .collect();
                        ScalarHandlerComponentCase {
            ...
            parts.push(ScalarHandlerParts { handler, cases });
            continue;
        }
        ...
        if assertions.iter().any(|initial| {
            initial.components.iter().any(|component| {
                !scalar_expression_matches_value(&handler.result, &component.initial_value)
            })
        }) {
            return Err(CanonicalSourceErrorV1::InvalidScalarHandler {
                origin: handler.origin,
...
fn state_relation_shape<'a>(
    ...
    let produced = relation
        ...
        .iter()
        .filter(|mode| mode.known.iter().any(|role| role == subject))
        .flat_map(|mode| &mode.produced)
        .collect::<BTreeSet<_>>();
    let produced = produced.iter().copied().collect::<Vec<_>>();
    let [value] = produced.as_slice() else {
        return None;
    };
    let domain = relation
    ...
    cst.items.iter().find_map(|item| match &item.kind {
        CstKind::Shape {
            designation,
            fields,
        } if designation == domain => Some((designation.as_slice(), fields.as_slice())),
        _ => None,
    })
}
