...
pub(super) struct ParsedSource {
    ...
    pub imported_sources: BTreeMap<CanonicalSourceArtifactIdV1, Box<[u8]>>,
}

pub(super) struct OriginEdit<'a> {
    old: CanonicalSourceArtifactIdV1,
    new: CanonicalSourceArtifactIdV1,
    start: u64,
    end: u64,
    length: u64,
    imports: &'a BTreeMap<CanonicalSourceArtifactIdV1, Box<[u8]>>,
}
...
impl OriginEdit<'_> {
    pub(super) fn origin(&self, origin: &mut CanonicalSourceOriginV1) -> Result<(), CanonicalSourceErrorV1> {
        ...
        if origin.artifact != self.old {
            return self.imports.get(&origin.artifact)
                .filter(|source| origin.end <= source.len() as u64)
                .map(|_| ()).ok_or(CanonicalSourceErrorV1::RecordedPlanMismatch);
        }
    ...
    fn item(&self, item: &mut CstItem) -> Result<(), CanonicalSourceErrorV1> {
        ...
        match &mut item.kind {
            CstKind::Referent { .. } | CstKind::Capability { .. } | CstKind::ForeignType { .. } => {},
...
pub(super) fn replace_scalar_leaf(
    ...
    let edit = OriginEdit { old: previous.artifact, new: artifact,
        ...
        length: replacement_length as u64, imports: &previous.imported_sources };
    ...
    for callable in &mut parsed.callables { callable.rebase_origins(&edit)?; }
