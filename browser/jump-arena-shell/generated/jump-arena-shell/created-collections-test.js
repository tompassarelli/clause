...
function object(value) {
    if (typeof value !== "object" || value === null || Array.isArray(value))
        throw new Error("expected projected object");
    return value;
}
const artifact = (name) => new URL(`../../../target/created-collections/${name}`, import.meta.url);
const bytes = async (name) => [...new Uint8Array(await file(artifact(name)).arrayBuffer())];
const scalar = (channel, value) => ({ kind: "scalar-input", channel, value });
test("fresh Wasm iterates exact created occurrences, preserves them through checked edits, and passively projects non-game sets", async () => {
    ...
    const open = async (name) => {
        ...
        const checked = completed(done => port.acceptPackage(request, done));
        if (checked._tag !== "PackageAccepted")
    ...
    const run = (inputs = [], milliseconds = 1000) => {
        ...
        if (pending._tag !== "CandidateProduced")
            throw new Error(pending.reason);
        const admitted = completed(done => port.requestAdmission(session, pending.candidate, done));
        if (admitted._tag !== "AdmissionAccepted")
            throw new Error(admitted.reason);
        return object(wasm["decode-projected-term-frame"](admitted.frame));
    };
    const rows = (frame, relation) => {
        const table = object(object(frame.relations)[relation]);
        expect(table.kind).toBe("relation-table");
        if (!Array.isArray(table.rows))
            throw new Error("expected exact rows");
        return table.rows.map(object);
    };
    ...
    expect(effects.every(effect => object(effect.subject).kind === "referent")).toBe(true);
    ...
    frame = run();
    expect(rows(frame, "burn-target")).toHaveLength(0);
    expect(object(frame["cinder-1"]).vitality).toBe(75.5);
    port.disposeSession(session);
    session = await open("goals.cwr1");
    revision = 0;
    sequence = 0;
    ...
    frame = run([], 500);
    expect(object(frame.account).balance).toBe(114);
    expect(object(frame.account)["known-goal"]).toHaveLength(1);
    frame = run();
    expect(object(frame.account).balance).toBe(121);
    port.disposeSession(session);
}, 60_000);
//# sourceMappingURL=created-collections-test.js.map