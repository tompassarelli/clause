...
import { conj_value as $$bc$conj_value, equivV as $$bc$equiv, keyword as $$bc$keyword, property_key as $$bc$property_key, record_instance_p as $$bc$record_instance_p } from 'beagle/core.js';
...
function exact_module_bytes_p(value) {
  if ((value == null)) {
    ...
    return ((_logical) => (_logical !== false && _logical != null ? ((_logical) => (_logical !== false && _logical != null ? ((1 <= length) && (length <= wasm_module_max_bytes)) : _logical))(Number.isSafeInteger(length)) : _logical))(($$bc$record_instance_p("jump-arena-shell.wasm-workbench-shell/ArrayBuffer", value) || ArrayBuffer.isView(value)));
  }
}
...
function create_composition_bang(create_shell, browser, module, exact_request_bytes, fixed_tick, policy, emit_receipt) {
...
if ((!(active == null))) {
  ...
  const active = workbench["create-cartridge-workbench!"](port, fixed_tick, policy, (delay, tick) => schedule_fixed_tick(browser, delay, tick), (values) => (passive_shell.renderFrame)(project_cwo1_arena_frame(values)), emit_receipt, wasm["->ExactProcessRequest"](exact_request_bytes));
...
function create_passive_wasm_workbench_shell_bang(mount, browser, three, module, exact_request_bytes, fixed_tick, policy, emit_receipt) {
  return create_composition_bang((emit_input) => shell["create-jump-arena-shell!"](mount, browser, three, emit_input), browser, module, exact_request_bytes, fixed_tick, policy, emit_receipt);
}

function load_passive_wasm_workbench_shell_bang(initialize_module, exact_module_bytes, mount, browser, three, exact_request_bytes, fixed_tick, policy, emit_receipt) {
  return initialize_module({[$$bc$property_key($$bc$keyword("module_or_path"))]: require_module_bytes(exact_module_bytes)}).then((module) => create_passive_wasm_workbench_shell_bang(mount, browser, three, module, exact_request_bytes, fixed_tick, policy, emit_receipt));
}
