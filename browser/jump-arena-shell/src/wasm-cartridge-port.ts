...
function projected_table(payload: CanonicalBytes): ProjectedValue {
  ...
  const take = (count: number) => {
    ...
    const value = Array.from({ length: count }, (_, i) => byte_at(payload, offset + i)); offset += count; return value;
  };
  const u8 = () => take(1)[0];
  ...
  const referent = (): ProjectedReferent => {
    ...
    if (tag === 1) return checked_referent({ kind: "referent", domain, identity: { kind: "created", value: take(32) } });
  ...
  for (let row = 0; row < count; ++row) {
    ...
    for (let i = 0; i < valueCount; ++i) {
      ...
      if (valueKind === 0 && tag === 0) value = projected_number(take(8));
