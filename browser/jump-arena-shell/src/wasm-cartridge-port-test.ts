...
test["test"](
  ...
  () =>
    Promise.all([
      ...
      test["expect"](concatenate(retired_result.reason)).toBe(
        "Wasm session is disposed",
      );
      test["expect"](
        concatenate(
...
test["test"](
  ...
  () =>
    Promise.all([
      file("./generated/wasm/clause_runtime_bg.wasm").arrayBuffer(),
      file(
        "./fixtures/wasm-ongoing-effect-v1/ongoing-effect-v1.cwr1.hex",
      ).text(),
    ]).then((assets) => {
      const module = initialize_real_session_module(assets[0]);
      const port = wasm["create-wasm-cartridge-port"](module, policy());
