...
interface MountLike {
  ...
  appendChild(canvas: ArenaCanvas): void;
}
...
interface RendererLike {
  readonly domElement: ArenaCanvas;
...
interface ThreeLike {
  ...
  readonly Color: new (color: number) => unknown;
  ...
  readonly MeshStandardMaterial: new (options: {
...
function is_property_bag(value: unknown): value is PropertyBag {
  return (
...
function is_frozen_vec3(value: unknown): value is ArenaVec3 {
  return (
    is_property_bag(value) &&
...
function is_frozen_player_frame(value: unknown): value is ArenaPlayerFrame {
  return (
    is_property_bag(value) &&
    Object.isFrozen(value) &&
...
function is_frozen_platform_frame(
  value: unknown,
): value is ArenaPlatformFrame {
  return (
    is_property_bag(value) &&
    Object.isFrozen(value) &&
    is_frozen_vec3(value.position) &&
...
function is_frozen_collectible_frame(
  value: unknown,
): value is ArenaCollectibleFrame {
  return (
    is_property_bag(value) &&
    Object.isFrozen(value) &&
    is_frozen_vec3(value.position) &&
...
function is_frozen_world_frame(value: unknown): value is ArenaWorldFrame {
  if (!is_property_bag(value) || !Object.isFrozen(value)) return false;
...
function is_frozen_frame(value: unknown): value is ArenaFrame {
  return (
    is_property_bag(value) &&
    Object.isFrozen(value) &&
...
function require_frozen_frame(value: unknown): ArenaFrame {
  if (is_frozen_frame(value)) return value;

  throw new Error("renderFrame requires a deeply frozen arena frame");
}

function is_mount_like(value: unknown): value is MountLike {
  return (
    is_property_bag(value) &&
...
function require_mount(value: unknown): MountLike {
  if (is_mount_like(value)) return value;
  throw new Error("jump arena shell requires a mount");
}

function is_browser_like(value: unknown): value is BrowserLike {
  return (
    is_property_bag(value) &&
...
function require_browser(value: unknown): BrowserLike {
  if (is_browser_like(value)) return value;
  throw new Error("jump arena shell requires a browser host");
}

function is_three_like(value: unknown): value is ThreeLike {
  return (
    is_property_bag(value) &&
...
function require_three(value: unknown): ThreeLike {
  if (is_three_like(value)) return value;
  throw new Error("jump arena shell requires a Three.js host");
}

function numeric_value(value: unknown): number {
  return Number.parseFloat(String(value));
}

function projected_symbol_color(value: string): number {
  let hash = 216613;
...
function create_jump_arena_shell_bang(
  mount_value: unknown,
  browser_value: unknown,
  three_value: unknown,
  emit_input: (input: ArenaInput) => unknown,
): JumpArenaShell {
  ...
  let has_frame = false;
  let platform_meshes: MeshLike[] = [];
  let collectible_meshes: MeshLike[] = [];
  ...
  const resize = (): void => {
    ...
    if (has_frame) renderer.render(scene, camera);
  };

  const focus_canvas = (): void => {
    if (!disposed) canvas_focused = true;
  };

  const blur_canvas = (): void => {
    if (!disposed) canvas_focused = false;
  };

  const key_down = (event: KeyboardEventLike): unknown => {
    if (!disposed && canvas_focused) {
      return emit_input(keyboard_input("down", event));
    }
  };

  const key_up = (event: KeyboardEventLike): unknown => {
    if (!disposed && canvas_focused) {
  ...
  const pointer_event = (
    phase: "down" | "move" | "up" | "cancel",
    event: PointerEventLike,
  ): unknown => emit_input(pointer_input(phase, event, canvas));

  const pointer_down = (event: PointerEventLike): unknown => {
    if (disposed) return;
  ...
  const pointer_move = (event: PointerEventLike): unknown => {
    if (!disposed) return pointer_event("move", event);
  };

  const release_pointer = (event: PointerEventLike): void => {
  ...
  const pointer_up = (event: PointerEventLike): unknown => {
    if (disposed) return;
  ...
  const pointer_cancel = (event: PointerEventLike): unknown => {
    if (disposed) return;
  ...
  const render_frame = (incoming: unknown): ArenaFrame => {
    if (disposed) throw new Error("jump arena shell is disposed");
    ...
    for (const mesh of platform_meshes) platform_group.remove(mesh);
    platform_meshes = [];
    ...
    camera.position.set(
      position.x + Math.sin(yaw) * 7.5,
      position.y + 4.8,
      position.z + Math.cos(yaw) * 7.5,
    );
