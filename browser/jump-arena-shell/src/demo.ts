...
interface SessionModule {
  ...
  readonly clause_session_v1_event_bulk: () => Uint8Array;
  readonly clause_session_v1_reclaim_retired: () => boolean;
}

function demo_policy(): workbench.WorkbenchPolicy {
  return workbench["->WorkbenchPolicy"](
    ...
    workbench["->WorkbenchSequenceLimits"](64, 16, 8, 8, 16),
  );
}

async function fetch_text_bang(path: string): Promise<string> {
...
async function fetch_bytes_bang(path: string): Promise<ArrayBuffer> {
  const response = await fetch(path);
  if (!response.ok) {
    throw new Error(`Unable to load ${path}`);
  }
...
function initialize_session_module(module: ArrayBuffer): SessionModule {
  initSync(module);
...
function require_html_element(selector: string): HTMLElement {
...
type DemoComposition = ReturnType<
  (typeof integration)["create-passive-wasm-workbench-shell!"]
>;

let active_composition: DemoComposition | null = null;
...
async function load_demo_bang(): Promise<null> {
  ...
  const composition = integration["create-passive-wasm-workbench-shell!"](
    arena,
    window,
    three_module,
    module,
    wasm["decode-cwr1-hex"](fixture_text),
