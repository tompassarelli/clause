...
function object(value) {
    if (typeof value !== "object" || value === null || Array.isArray(value))
        throw new Error("expected diagnostic object");
    return value;
}
const artifact = (name) => new URL(`../../../target/live-semantics/${name}`, import.meta.url);
...
test("fresh live Wasm preserves actual encounter, explains executed contributions, and searches isolated finite interventions", async () => {
    ...
    const accepted = completed(done => port.acceptPackage(wasm["->ExactProcessRequest"](initialBytes), done));
    if (accepted._tag !== "PackageAccepted")
        throw new Error(accepted.reason);
    const started = completed(done => port.startSession(accepted.acceptedPackage, 1, done));
    ...
    let revision = 0;
    let sequence = 0;
    const candidate = (values = [], milliseconds = 16) => {
    ...
    const admit = (pending) => {
        ...
        const result = completed(done => port.requestAdmission(session, pending.candidate, done));
    ...
    expect(oldFacets).toHaveLength(2);
    expect(newFacets).toHaveLength(2);
    ...
    for (const [coordinate, incoming] of Object.entries(states)) {
        const state = object(incoming);
        const source = object(state.source);
        if (source.relation === "selected")
            allowed.push({ slot: Number(coordinate), value: false });
        if (source.subject === "cinder-1" && source.relation === "vitality")
...
//# sourceMappingURL=live-semantics-test.js.map