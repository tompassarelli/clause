...
export function firstDifference(expected, actual, path = []) {
    ...
    for (const key of new Set([...left.keys(), ...right.keys()])) {
        if (left.has(key) !== right.has(key))
            return { path: [...path, key], expected: left.get(key), actual: right.get(key) };
        const difference = firstDifference(left.get(key), right.get(key), [...path, key]);
        if (difference)
            return difference;
    }
}
function byteDifference(expected, actual) {
    for (let i = 0; i < Math.max(expected.length, actual.length); ++i) {
        if (expected[i] !== actual[i])
            return i;
    }
}
export function compareEvents(expected, actual) {
...
export async function replay(directory, modulePath) {
    const artifact = (name) => resolve(directory, name);
    const bytes = async (name) => new Uint8Array(await Bun.file(artifact(name)).arrayBuffer());
    ...
    const sourceState = (difference) => {
        const states = fields(fields(metadata)?.get("states"));
        if (!states || difference.path[0] !== "projection")
            return;
        for (const page of states.values())
            for (const state of fields(page)?.values() ?? []) {
                const values = fields(state);
                if (values?.get("subject") === difference.path[1] && values.get("relation") === difference.path[2])
                    return state;
            }
    };
    let events = 0, admissions = 0;
    const mismatch = (inputIndex, command, phase, result) => ({
    ...
    if (status !== 0)
        throw new Error(`initial session rejected with status ${status}; source=${source}`);
    const openedBytes = module.clause_session_v1_event_bulk();
    const opened = decodeSessionEvent([...openedBytes]);
    if (opened.kind !== "opened")
        throw new Error(`initial session returned ${opened.kind}`);
    const openingDifference = compareEvents(await bytes("opened.cse1"), openedBytes);
    if (openingDifference)
        return { inputs: inputs.length, events, admissions, mismatch: mismatch(null, null, "open", openingDifference) };
    ...
    if (offset !== undefined)
        return { inputs: inputs.length, events, admissions, mismatch: mismatch(null, null, "initial projection", {
                byteOffset: offset,
    ...
    for (const line of (await Bun.file(artifact("commands.tsv")).text()).trim().split("\n")) {
        ...
        if (!Number.isSafeInteger(ordinal) || ordinal !== events || !Number.isSafeInteger(index) || index < 0 || !phase)
            throw new Error("invalid replay command index");
        const status = module.clause_session_v1_command_bulk(await bytes(`${ordinal}.cwi1`));
        if (status !== 0)
            throw new Error(`input ${index} (${inputs[index] ?? phase}), command ${ordinal}: Wasm rejected status ${status}; source=${source}`);
        const actual = module.clause_session_v1_event_bulk();
        const result = compareEvents(await bytes(`${ordinal}.cse1`), actual);
        events += 1;
        if (result)
            return { inputs: inputs.length, events, admissions, mismatch: mismatch(index, ordinal, phase, result) };
        if (phase === "admit")
            admissions += 1;
    }
...
if (import.meta.main) {
    ...
    if (!directory || !modulePath || Bun.argv.length !== 4)
        throw new Error("usage: bun clause:browser/runtime/src/game-replay.ts RECORD_DIRECTORY WASM_JS_MODULE");
    const result = await replay(directory, modulePath);
    console.log(JSON.stringify(result, null, 2));
    if (result.mismatch)
        process.exitCode = 1;
}
//# sourceMappingURL=game-replay.js.map