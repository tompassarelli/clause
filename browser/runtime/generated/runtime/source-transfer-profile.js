...
function completed(action) {
    ...
    if (values.length !== 1)
        throw new Error("expected one boundary completion");
    return values[0];
}
...
for (const name of ["encounter", "collections"]) {
    ...
    for (let index = 0; index <= samples * 2; ++index) {
        ...
        const checked = completed(done => port.acceptPackage(wasm["->ExactProcessRequest"](initial), done));
        ...
        if (name === "collections")
            values.push(...[1, 3].map(value => ({ kind: "scalar-input", channel: "IgniteDuration", value })));
        ...
        const candidate = completed(done => port.runCandidate(started.session, workbench["->FixedTick"](16), workbench["->InputConfiguration"](1, inputs), done));
        if (candidate._tag !== "CandidateProduced")
            throw new Error(candidate.reason);
        const admitted = completed(done => port.requestAdmission(started.session, candidate.candidate, done));
        if (admitted._tag !== "AdmissionAccepted")
            throw new Error(admitted.reason);
        if (profiled && !module.clause_source_profile_v1_begin())
            throw new Error("profile already active");
        if (profiled && !beginSourceTransferObservation())
            throw new Error("outer profile already active");
        ...
        const profile = profiled ? JSON.parse(module.clause_source_profile_v1_finish()) : null;
        if (result._tag !== "SessionStarted")
            throw new Error(result.reason);
        if (profiled && object(profile).truncated !== false)
            throw new Error("incomplete phase evidence");
        if (profiled && (!outerProfile || outerProfile.truncated))
            throw new Error("incomplete outer phase evidence");
...
//# sourceMappingURL=source-transfer-profile.js.map