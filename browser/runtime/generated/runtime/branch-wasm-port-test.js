...
function initialize_real_branch_module(module) {
    ...
    return Object.freeze({
        ...
        clause_branch_v1_request_push: (byte) => clause__branch__v1__request__push(byte),
        ...
        clause_branch_v1_event_byte: (index) => clause__branch__v1__event__byte(index),
    });
}
...
test["test"]("real Wasm forks reconnects admits and explains one construct-blind process", () => Promise.all([
    Bun.file("./generated/wasm/clause_runtime_bg.wasm").arrayBuffer(),
    Bun.file("../../test-vectors/browser/wasm-process-continuation-v1/process-continuation-v1.cwr1.hex").text(),
]).then((assets) => {
    ...
    test["expect"](!(json_string(authoritative.run) === json_string(branch_run))
        ? "true"
        : "false").toBe("true");
    ...
    test["expect"](json_string(branch_command.occurrence)).toBe(json_string(occurrences[0]));
    ...
    test["expect"](!(json_string(admitted.successor) === json_string(r1))
        ? "true"
        : "false").toBe("true");
    ...
    test["expect"](json_string(explained_branch_command)).toBe(json_string(branch_command));
    test["expect"](json_string(authoritative_command.occurrence)).toBe(json_string(occurrences[0]));
    test["expect"](!(json_string(authoritative_command.step) ===
        json_string(branch_command.step))
        ? "true"
        : "false").toBe("true");
    test["expect"](!(json_string(authoritative_command.observation) ===
    ...
    test["expect"](concatenate(branch["dispose-process-branch!"](module, process_branch))).toBe("true");
    return null;
}));
