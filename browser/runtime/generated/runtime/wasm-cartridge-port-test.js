...
test.test("source continuity decodes the complete CSC1 map with exact identities and bounds", () => {
    ...
    for (const [old, next, first, occurrence] of [[0xffffffff, 19, 23, 3], [17, 0, 0xffffffff, 4]]) {
        append_u32_bang(bytes, old);
        append_u32_bang(bytes, next);
        bytes.push(...identity(5));
        append_u32_bang(bytes, first);
        bytes.push(...identity(occurrence));
    }
    ...
    const hex = (tag) => identity(tag).map(byte => byte.toString(16).padStart(2, "0")).join("");
...
test["test"]("real Wasm suspends resumes proposes and admits one domain-neutral process", () => Promise.all([
    file("./generated/wasm/clause_runtime_bg.wasm").arrayBuffer(),
    file("../../test-vectors/browser/wasm-process-continuation-v1/process-continuation-v1.cwr1.hex").text(),
]).then((assets) => {
...
test["test"]("real Wasm executes a generically allocated Clause source plan", () => Promise.all([
    file("./generated/wasm/clause_runtime_bg.wasm").arrayBuffer(),
    file("../../test-vectors/browser/wasm-generic-source-v1/generic-source-v1.cwr1.hex").text(),
]).then((assets) => {
    const port = wasm["create-wasm-cartridge-port"](initialize_real_session_module(assets[0]), arena_policy());
    const request = wasm["->ExactProcessRequest"](wasm["decode-cwr1-hex"](assets[1]));
    const accepted = acceptPackage(port, request);
...
test["test"]("real Wasm refuses an effect before intent admission", () => Promise.all([
    file("./generated/wasm/clause_runtime_bg.wasm").arrayBuffer(),
    file("../../test-vectors/browser/wasm-ongoing-effect-v1/ongoing-effect-v1.cwr1.hex").text(),
]).then((assets) => {
    const module = initialize_real_session_module(assets[0]);
    const port = wasm["create-wasm-cartridge-port"](module, policy());
    const request = wasm["->ExactProcessRequest"](wasm["decode-cwr1-hex"](assets[1]));
...
test.test("projection cursor preserves object field custody and rejects malformed tails", () => {
    const text = (value) => projected_atom_node("clause/process-projected-text-v1", [...new TextEncoder().encode(value)]);
    const end = projected_atom_node("clause/js-object-end-v1", []);
    const field = (key, value, rest) => projected_triple_node(projected_atom_node("clause/js-field-v1", [...new TextEncoder().encode(key)]), value, rest);
    const frame = (term) => new Array(64).fill(0).concat(term);
    ...
    const binary = (bytes) => bytes.map(byte => String.fromCharCode(byte)).join("");
