...
function projected_set(node: TermNode): ProjectedValue {
  if (node.kind !== "triple") throw new Error("invalid projected set");
  const [header, tree, end] = node.slots;
  if (header.kind !== "atom" || header.payload.length !== 1 || ![0, 1, 2, 6, 8].includes(byte_at(header.payload, 0)) ||
      end.kind !== "atom" || atom_kind_text(end) !== "clause/process-projected-set-end-v1" || end.payload.length !== 0) throw new Error("invalid projected set header");
  const values: ProjectedValue[] = [];
  const kind = byte_at(header.payload, 0);
  const visit = (tree: TermNode, depth: number): void => {
    if (depth > 32 || values.length > 65535) throw new Error("projected set exceeds bounds");
    if (tree.kind === "atom") {
      if (atom_kind_text(tree) !== "clause/process-projected-set-end-v1" || tree.payload.length !== 0) throw new Error("invalid projected set tree");
      return;
    }
    ...
    if (leaf.kind !== "atom" || atom_kind_text(leaf) !== expected.get(kind)) throw new Error("wrong projected set kind");
    ...
    values.push(value); visit(tree.slots[2], depth + 1);
  };
  visit(tree, 0); return Object.freeze(values);
}
...
function realize_projection_node(node: TermNode): ProjectedValue {
  if (node.kind === "atom") {
    ...
    if (kind === "clause/process-projected-referent-v1") {
      ...
      if (payload.length === 37 && byte_at(payload, 4) === 1) return checked_referent({ kind: "referent", domain: little_u32(bytes, 0), identity: { kind: "created", value: frozen_byte_range(bytes, 5, 37) } });
    ...
    if (kind === "clause/process-projected-set-v1") return projected_set(node);
