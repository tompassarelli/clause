...
function decode_hex_octets(
  ...
  for (let index = 0; index < source.length; index += 1) {
    ...
    if (high < 0) {
      ...
      if (length >= maximumBytes) throw new Error(`${label} hex transport exceeds its byte bound`);
      bytes[length++] = high * 16 + nibble;
      high = -1;
    }
  }
  if (high >= 0) throw new Error(`${label} hex transport has an incomplete byte`);
  if (length === 0) throw new Error(`${label} hex transport is empty`);
...
function projected_scalar_order(a: ProjectedValue, b: ProjectedValue, kind?: number): number {
  if (typeof a === "number" && typeof b === "number") {
    const bits = (n: number) => { const bytes = new DataView(new ArrayBuffer(8)); bytes.setFloat64(0, n, true); return bytes.getBigUint64(0, true); };
    return bits(a) < bits(b) ? -1 : bits(a) > bits(b) ? 1 : 0;
  }
  if (typeof a === "boolean" && typeof b === "boolean") return Number(a) - Number(b);
  if (typeof a === "string" && typeof b === "string") {
    const encoder = new TextEncoder(); const left = encoder.encode(a), right = encoder.encode(b);
    if (kind === 2 && left.length !== right.length) return left.length - right.length;
  ...
  if (left.identity.kind !== right.identity.kind) return left.identity.kind === "declared" ? -1 : 1;
  if (left.identity.kind === "declared" && right.identity.kind === "declared") return left.identity.value - right.identity.value;
  if (left.identity.kind === "created" && right.identity.kind === "created") {
    for (let i = 0; i < 32; ++i) { if (left.identity.value[i] !== right.identity.value[i]) return left.identity.value[i] - right.identity.value[i]; }
    return 0;
  }
  throw new Error("incompatible projected scalar values");
}

function projected_table(payload: CanonicalBytes): ProjectedValue {
  let offset = 0;
  const advance = (count: number) => {
    if (count < 0 || offset + count > payload.length) throw new Error("truncated projected relation table");
    const start = offset; offset += count; return start;
  };
  const take = (count: number) => canonical_byte_range(payload, advance(count), offset);
  const u8 = () => byte_at(payload, advance(1));
  const u16 = () => little_u16(payload, advance(2));
  const u32 = () => little_u32(payload, advance(4));
  const referent = (): ProjectedReferent => {
    const domain = u32(), tag = u8();
    if (tag === 0) return checked_referent({ kind: "referent", domain, identity: { kind: "declared", value: u32() } });
    if (tag === 1) return checked_referent({ kind: "referent", domain, identity: { kind: "created", value: frozen_byte_range(payload, advance(32), offset) } });
    throw new Error("invalid projected row referent");
  };
  if (payload.length > 1024 * 1024 || u8() !== 6) throw new Error("invalid projected relation table");
  ...
  for (let row = 0; row < count; ++row) {
    ...
    if (subject.domain !== subjectDomain || (previous && projected_scalar_order(previous, subject) >= 0)) throw new Error("noncanonical projected relation subjects");
    previous = subject;
    const valueCount = u16();
    if (valueCount === 0 || (cardinality !== 2 && valueCount !== 1)) throw new Error("invalid projected row cardinality");
    const values: ProjectedValue[] = [];
    for (let i = 0; i < valueCount; ++i) {
      const tag = u8(); let value: ProjectedValue;
      if (valueKind === 0 && tag === 0 && numericView !== null) {
        value = numericView.getFloat64(advance(8), true);
        if (!Number.isFinite(value) || Object.is(value, -0)) throw new Error("CWO1 number is not canonical finite f64");
      }
      else if (valueKind === 1 && tag === 1) { const bit = u8(); if (bit > 1) throw new Error("invalid projected Boolean"); value = bit === 1; }
      else if (valueKind === 2 && tag === 2) {
        const count = u8(); if (count === 0 || count > 64) throw new Error("invalid projected symbol length");
        value = ascii_text(take(count), "projected symbol");
      }
      else if (valueKind === 3 && tag === 4) value = utf8_text(take(u16()), "projected Text");
      else if (valueKind === 4 && tag === 5) { const reference = referent(); if (reference.domain !== valueDomain) throw new Error("wrong projected value domain"); value = reference; }
      else throw new Error("wrong projected row value kind");
      if (i > 0 && projected_scalar_order(values[i - 1], value, valueKind) >= 0) throw new Error("noncanonical projected row values");
      values.push(value);
    }
...
function realize_projected_atom(kind: string, payload: CanonicalBytes): ProjectedValue {
  ...
  if (kind === "clause/process-projected-f64-v1") {
    if (payload.length !== 8) throw new Error("projected F64 payload is invalid");
    return finite_f64(payload, 0);
  }
  if (kind === "clause/process-projected-bool-v1") {
    const value = byte_at(payload, 0);
    if (payload.length !== 1 || value > 1) throw new Error("projected Boolean payload is invalid");
    return value === 1;
  }
  if (kind === "clause/process-projected-symbol-v1") return ascii_text(payload, "projected symbol");
  if (kind === "clause/process-projected-text-v1") return utf8_text(payload, "projected Text");
  if (kind === "clause/process-projected-relation-table-v1") return projected_table(payload);
  if (kind === "clause/process-projected-referent-v1") {
    if (payload.length === 9 && byte_at(payload, 4) === 0) return checked_referent({ kind: "referent", domain: little_u32(payload, 0), identity: { kind: "declared", value: little_u32(payload, 5) } });
    if (payload.length === 37 && byte_at(payload, 4) === 1) return checked_referent({ kind: "referent", domain: little_u32(payload, 0), identity: { kind: "created", value: frozen_byte_range(payload, 5, 37) } });
    throw new Error("projected referent is malformed");
  }
  throw new Error("projected scalar Atom is not realizable");
}

interface CachedProjectedValue {
  readonly end: number;
  readonly depth: number;
  readonly value: ProjectedValue;
}
interface CachedProjectedFrame {
  readonly source: string;
  readonly values: Map<number, CachedProjectedValue>;
}
...
let previousProjectedFrame: CachedProjectedFrame | undefined;

class ProjectionCursor {
  offset = 2 * identity_bytes;
  readonly values = new Map<number, CachedProjectedValue>();
  maximumDepth = 0;
  constructor(readonly bytes: CanonicalBytes, readonly retainValues: boolean, readonly previousFrame?: CachedProjectedFrame) {
    require_range(bytes, 0, this.offset, "projected Term scope");
  }
  tag(depth: number): number {
    if (depth > cse1_projected_term_max_depth) throw new Error("projected Term exceeds its depth bound");
    if (this.retainValues) this.maximumDepth = Math.max(this.maximumDepth, depth);
    require_range(this.bytes, this.offset, 1, "projected Term node");
    const tag = byte_at(this.bytes, this.offset++);
    if (tag > 1) throw new Error("projected Term node tag is invalid");
    return tag;
  }
  atomPayload(): { kind: string; payload: CanonicalBytes } {
    ...
    if (byte_at(this.bytes, payload.next) !== 0) throw new Error("projected Atom equality contract is invalid");
    return { kind: ascii_text(kind.bytes, "projected Atom kind"), payload: payload.bytes };
  }
  atom(depth: number): { kind: string; payload: CanonicalBytes } {
    if (this.tag(depth) !== 0) throw new Error("projected realization expected an Atom");
    return this.atomPayload();
  }
  value(depth: number): ProjectedValue {
    if (!this.retainValues || typeof this.bytes !== "string") return this.freshValue(depth);
    ...
    if (frame !== undefined && previous !== undefined &&
      ...
      if (maximumDepth > cse1_projected_term_max_depth) throw new Error("projected Term exceeds its depth bound");
  ...
  freshValue(depth: number): ProjectedValue {
    if (this.tag(depth) === 0) {
      const atom = this.atomPayload();
      return realize_projected_atom(atom.kind, atom.payload);
    }
    let head = this.atom(depth + 1);
    if (head.kind === "clause/process-projected-set-v1") return this.set(head.payload, depth);
    const object = head.kind === "clause/js-field-v1";
    if (!object && head.kind !== "clause/js-item-v1") throw new Error("projected Term lacks a realizable shape");
    const fields: Array<readonly [string, ProjectedValue]> = [];
    const values: ProjectedValue[] = [];
    const keys = new Set<string>();
    for (;;) {
      if (object) {
        if (head.kind !== "clause/js-field-v1") throw new Error("projected object entry lacks a field Atom");
        const key = ascii_text(head.payload, "projected field");
        if (keys.has(key)) throw new Error("projected object field is duplicated");
        keys.add(key);
        fields.push([key, this.value(depth + 1)]);
      } else {
        if (head.kind !== "clause/js-item-v1") throw new Error("projected array entry lacks an item Atom");
  ...
  set(payload: CanonicalBytes, depth: number): ProjectedValue {
    ...
    if (payload.length !== 1 || !kinds.has(byte_at(payload, 0))) throw new Error("invalid projected set header");
    const kind = byte_at(payload, 0), values: ProjectedValue[] = [];
    const end = (atom: { kind: string; payload: CanonicalBytes }) => {
      if (atom.kind !== "clause/process-projected-set-end-v1" || atom.payload.length !== 0) throw new Error("invalid projected set tree");
    };
    const visit = (depth: number, treeDepth: number): void => {
      if (treeDepth > 32 || values.length > 65535) throw new Error("projected set exceeds bounds");
      if (this.tag(depth) === 0) { end(this.atomPayload()); return; }
      visit(depth + 1, treeDepth + 1);
      const leaf = this.atom(depth + 1);
      if (leaf.kind !== kinds.get(kind)) throw new Error("wrong projected set kind");
      const value = realize_projected_atom(leaf.kind, leaf.payload);
      if (values.length && projected_scalar_order(values[values.length - 1], value, kind) >= 0) throw new Error("noncanonical projected set");
      values.push(value);
      visit(depth + 1, treeDepth + 1);
    };
    visit(depth + 1, 0);
...
function decode_projected_value(bytes: unknown, maximumBytes: number, reuseValues = false): ProjectedValue {
  ...
  if (cursor.offset !== source.length) throw new Error("projected Term has trailing bytes");
  if (reuseValues) previousProjectedFrame = typeof source === "string" ? { source, values: cursor.values } : undefined;
  return value;
}
...
interface SourcePreparationWasmModule extends SessionWasmModule {
  readonly clause_session_v1_prepare_source: (slot: number, generation: number, sequence: bigint, preparation: Uint8Array<ArrayBuffer>) => number;
}

function isSourcePreparationModule(module: unknown): module is SourcePreparationWasmModule {
...
export function decodeSourcePreparationHex(source: unknown): ExactBytes {
...
export function prepareSourceSession(module: unknown, incomingSession: unknown, preparation: ExactBytes): void {
...
export function sourceContinuity(module: unknown, incomingSession: unknown): ProjectedValue {
  ...
  const hex = (start: number): string => {
    let value = "";
    for (let index = start; index < start + 32; index++) value += bytes[index].toString(16).padStart(2, "0");
    return value;
  };
  const formations: Array<readonly [string, ProjectedValue]> = [];
  let offset = 76;
  ...
  const slots: Array<readonly [string, ProjectedValue]> = [];
  const seenSlots = new Set<number>();
  for (let index = 0; index < slotCount; index++, offset += 4) {
    const old = view.getUint16(offset, true);
    if (seenSlots.has(old)) throw new Error("source continuity duplicates a slot");
    seenSlots.add(old);
    slots.push([String(old), view.getUint16(offset + 2, true)]);
  }
  const paged = (entries: readonly (readonly [string, ProjectedValue])[]): ProjectedObject => {
    const pages = new Map<number, Array<readonly [string, ProjectedValue]>>();
    for (const [key, value] of entries) {
      const index = Number(key), page = Math.floor(index / 64);
      let fields = pages.get(page);
      if (fields === undefined) { fields = []; pages.set(page, fields); }
      fields.push([String(index % 64), value]);
    }
    return Object.freeze(Object.fromEntries([...pages].map(([page, fields]) =>
      [String(page), Object.freeze(Object.fromEntries(fields))])));
  };
