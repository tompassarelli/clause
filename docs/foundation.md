...
> **Status:** Current.
>
> **Authority:** Normative semantic foundation for Clause.
>
> **Relationship:** Governs the [target surface](surface.md) and
> [roadmap](roadmap.md). Supersedes foundations built from primitive types,
> values, objects, fields, functions, or sets. The
> [historical M0 checkpoint](history/m0.md) no longer governs current syntax.

## Scope

This document is Clause's semantic authority. The target surface defines
authoring projection, and the roadmap orders implementation. Neither may
introduce a second semantic ontology.
...
`x: form` is binding/definition. It orients the stable handle `x` to the denotation
...
## Binding, membership, and equality
...
`x ∈ T` is ordinary membership relational content, with `member` and `group`
as named roles. It does not declare a primitive host type or collapse identity
through equality. `x: form` establishes a stable binding or definition. `=`
authors equality content. A human-facing editor may transform typed `::` into
`∈` before parsing; raw `::` and word aliases are not Clause grammar.
...
`x <= y`, `x = y`, `x != y`, `a + b`, `a - b`, `a * b`, and `a / b` are canonical;
...
`+` and `-` are arithmetic relations. Spacing distinguishes division from
slash-qualified semantic names such as `egress/route`.
...
persistence is now
`clause-semantic-v8` inside `clause-revision-v5`, with exact predecessor
...
Implementation evidence must make every distinction above explicit in checked
representations, diagnostics, and executable behavior. A lossless reader stays
semantic-free. Later elaboration may classify source forms but may not invent
referents, judgments, authority, negation, execution, or host types.
